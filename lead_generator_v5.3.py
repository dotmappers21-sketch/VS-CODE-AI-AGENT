#!/usr/bin/env python3
"""
Lead Generation Automation Tool — V5.3
=======================================
...
V5.3 Enhancements (PRACTICAL full-name resolution — uses data we already have):
- Company name extraction: "Matthew Cornell Photography" → "Matthew Cornell" for lead "Matthew"
- Domain name extraction: "matthewcornell.com.au" → "Matthew Cornell" for lead "Matt"/"Matthew"
- LinkedIn URL slug parsing: "/in/matthew-cornell-123" → "Matthew Cornell"
- Apollo person `name` field: use it directly when available (was being ignored)
- Improved email inference: handles abbreviated names (matt→matthew), more TLD patterns
- Final name resolution pass after ALL enrichment steps exhaust API-based methods

V5.2 Features (inherited):
- Fixed _lead_is_complete() to require two-word name
- Email-based name inference, scraped-name backfill, SerpApi name fallback
- _needs_full_name flag, warning log for remaining single names

V5.1 Features (inherited):
- ThreadPoolExecutor parallel processing (8 workers), skip-if-complete, optimized rate limiters

V5 Features (inherited):
- Contact-info sorting, personal email TOP CSV, Lusha fix
...
class LeadGeneratorApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Lead Generation Pro V5.3")
