#!/usr/bin/env python3
"""
Lead Generation Automation Tool — V5
=====================================
...
- Speed optimization: concurrent ThreadPoolExecutor domain enrichment (5 workers)
- Optimized API calls: skip redundant requests, reduced rate limiter intervals
- Fixed Lusha integration: always called, supports single-name leads, proper logging
- Reduced web scraping paths: 3 paths instead of 7
- Capped Apollo people per_page: 10 instead of 25
...
class ApolloClient:
    ...
    def __init__(self, api_key: str):
        self.api_key = api_key
        self.limiter = RateLimiter(0.4)  # V5: Optimized from 0.6
...
class LushaClient:
    ...
    def __init__(self, api_key: str):
        self.api_key = api_key
        self.limiter = RateLimiter(0.2)  # V5: Optimized from 0.3
...
class WebScraper:
    ...
    CONTACT_PATHS = [
        "", "/contact", "/about-us",  # V5: Optimized to 3 paths (was 12)
    ]
    ...
    def __init__(self, country_code: str = "AU"):
        ...
        self.limiter = RateLimiter(0.5)  # V5: Optimized from 0.8
...
class LeadGenerationPipeline:
    ...
    def _phase4_enrichment(self):
        ...
        self._log("Phase 4: Multi-source lead enrichment (V5: Optimized for speed)")
        self._log("   V5 optimizations: reduced rate limits, fewer web scrape paths, full-name enrichment")
        ...
        for i, domain in enumerate(self.domains):
            ...
            for ld in domain_leads:
                ...
                if needs_name or needs_email:
                    ...
                    if enriched:
                        ...
                        # V4: Also grab phone from Apollo enrich if available
            ...
            for ld in domain_leads:
                ...
                if name and " " not in name:  # Still has single-word name
                    # Try Apollo one more time with empty last name (may get better results)
            ...
            if lusha_company:
                ...
                if lusha_co_name:
                    ...
                    # Update all leads with Lusha company name
                    for ld in domain_leads:
                        if not ld.get("company"):
                            ld["company"] = lusha_co_name
                            ld["source"] += "+Lusha"  # Tag when company comes from Lusha

            # Step 4: Lusha person enrichment — V4: try for ALL leads with a name
            for ld in domain_leads:
                if not ld.get("name"):
                    continue
                parts = ld["name"].split()
                first_n = parts[0]
                last_n = parts[-1] if len(parts) > 1 else ""
                # V4: Try Lusha even with just first name (send empty last name)
                # Lusha may still return results with firstName + companyDomain
...
class LeadGeneratorApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Lead Generation Pro V5")
