#!/usr/bin/env python3
"""
Lead Generation Automation Tool — V5.2
=======================================
...
V5.2 Enhancements (full-name resolution):
- Fixed _lead_is_complete() to require two-word name — single-name leads no longer skip enrichment
- Email-based name inference: extract last names from email patterns (first.last@, firstlast@, domain)
- Scraped-name backfill: match web-scraped full names to single-name leads by first-name match
- SerpApi full-name fallback: Google search for "FirstName CompanyName" to find last names
- _needs_full_name flag on leads for tracking enrichment progress
- Warning log for leads that still have single names after all enrichment

V5.1 Features (inherited):
...
class LeadGenerationPipeline:
    ...
    def _infer_name_from_email(lead: dict) -> str:
        """V5.2: Try to infer the last name from a personal email address.
        e.g. matt@matthewcornell.com.au → domain=matthewcornell → last='Cornell'
             sarah.jones@company.com   → local=sarah.jones → last='Jones'
        ...
        # Pattern 1: local part is "first.last" → extract last
        if "." in local:
            parts = local.split(".")
            if parts[0] == first and len(parts) > 1 and len(parts[1]) > 1:
                return name + " " + parts[1].title()

        # Pattern 2: local part is "firstlast" (concatenated)
        if local.startswith(first) and len(local) > len(first) + 1:
            suffix = local[len(first):]
            if suffix.isalpha() and len(suffix) >= 2:
                return name + " " + suffix.title()

        # Pattern 3: email domain contains name (e.g. matthewcornell.com.au)
        # Strip TLDs and check if domain starts with first+something
        clean_domain = email_domain.replace(".com.au", "").replace(".com", "") \
                                   .replace(".net.au", "").replace(".co", "") \
                                   .replace(".org", "").replace(".net", "")
        if clean_domain.startswith(first) and len(clean_domain) > len(first) + 1:
            suffix = clean_domain[len(first):]
    ...
    def _lead_is_complete(lead):
        """A lead is complete ONLY if it has FULL name (two+ words) + email + phone.
        V5.2 FIX: Ensures single-name leads (e.g. 'Matt') are NEVER considered complete,
        so they always proceed through all enrichment steps to find the last name.
        """
        name = lead.get("name", "")
        has_full_name = bool(name) and " " in name  # must have at least first + last
        has_email = bool(lead.get("email"))
        has_phone = bool(lead.get("phone"))
        return has_full_name and has_email and has_phone
...
class LeadGeneratorApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Lead Generation Pro V5.2")
