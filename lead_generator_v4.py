#!/usr/bin/env python3
"""
Lead Generation Automation Tool — V4
=====================================
...
V4 Enhancements:
- Fixed Lusha integration (source now properly shows "+Lusha")
- Fixed "None" last names from API responses
- OpenAI-powered personal email verification
- Decision-maker grouping: top 3 per company first, then remaining leads
- Completeness-based sorting within groups
- Better API utilization from all sources
...
class ApolloClient:
    ...
    def search_people_by_domain(self, domain: str, per_page: int = 25) -> list[dict]:
...
class LeadGenerationPipeline:
    ...
    def _phase6_export(self) -> str:
        ...
        # ── V4 Scoring: completeness-based ──
        def _completeness_score(lead):
            """Score lead by completeness. Higher = more complete."""
            score = 0
            ...
            # Primary contact info tiers
            if has_phone and email_is_personal:
                score += 500   # Best: phone + personal email
            elif has_phone and has_email:
                score += 400   # Phone + generic email
            elif email_is_personal:
                score += 300   # Personal email only
            elif has_email:
                score += 250   # Generic email only
            elif has_phone:
                score += 200   # Phone only
...
class LeadGeneratorApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Lead Generation Pro V4")
